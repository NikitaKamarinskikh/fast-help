...
from keyboards.inline.customer_orders import orders_callback
...
from models import OrdersModel, WorkersModel, OrderTimestampsModel
...
async def show_order_candidates(callback: types.CallbackQuery, callback_data: dict):
    ...
    if not order.worker:
        await callback.message.answer(
            **(await _get_message_content(order_id, 0))
        )
    else:
        await callback.message.answer(
...
async def flip_candidate(callback: types.CallbackQuery, callback_data: dict):
    ...
    await callback.message.edit_text(
        **(await _get_message_content(order_id, candidate_number))
    )
...
async def _get_message_content(order_id: int, candidate_number: int):
    order = await OrdersModel.get_by_id(order_id)
